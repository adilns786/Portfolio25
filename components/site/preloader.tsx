...
export default function Preloader() {
  ...
  useEffect(() => {
    ...
    paths.forEach((path) => {
      ...
      (path as SVGPathElement).style.stroke = "#000000"; // black stroke
      (path as SVGPathElement).style.fill = "none";
      (path as SVGPathElement).style.strokeWidth = "2.5";
      ...
      {!done && (
        <motion.div
          ...
          className="fixed inset-0 z-[9999] flex items-center justify-center 
                     bg-gradient-to-b from-[#f9f9f9] to-[#e6e6e6] overflow-hidden"
        >
          {/* Main SVG container */}
          <div ref={containerRef} className="relative">
            {/* Subtle dark shadow glow */}
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 0.3, scale: 1 }}
              transition={{ duration: 1, ease: "easeOut" }}
              className="absolute inset-0 blur-3xl bg-black/10 scale-110"
            />
            ...
            <motion.div
              ...
              className="absolute inset-0 -m-12 border border-gray-300/40 rounded-full"
